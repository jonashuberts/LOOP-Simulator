...
export function tokenize(code) {
  ...
  while (cursor < code.length) {
    ...
    if (code.substring(cursor, cursor + 2) === '/*') {
      cursor += 2;
      while (cursor < code.length && code.substring(cursor, cursor + 2) !== '*/') {
        if (code[cursor] === '\n') {
          lineNum++;
        }
        cursor++;
    ...
    const monusMatch = code.substring(cursor).match(/^(?:-|−)(?:·|•|\*|\.|dot)/);
    if (monusMatch) {
    ...
    // Numbers
    const numberMatch = code.substring(cursor).match(/^\d+/);
    if (numberMatch) {
    ...
    const labelMatch = code.substring(cursor).match(/^M\d+\b/i);
    if (labelMatch) {
    ...
    const identMatch = code.substring(cursor).match(/^[a-zA-Z_][a-zA-Z0-9_]*/);
    if (identMatch) {
  ...
  tokens.push({ type: 'EOF', value: 'EOF', line: lineNum });
  return tokens;
}
